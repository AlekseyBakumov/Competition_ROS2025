...
class Controller(Node):
    ...
    # когад свеетофор присылает msg.linear.x > 0.0 разрешаем машинке двигаться
    def cb_traffic(self, msg: Twist):
        self.last_tl_time = self.get_clock().now()
        if msg.linear.x > 0.0:
            self.allowed_to_move = True

    # детектим момент когда встретили знак дорожных работ чтобы остановит машинку
    ...
    def loop(self):
        ...
        # светофор. пока не разрешили двигаться стоим
        ...
        if self.last_sign_time is not None:
            # Считаем, сколько времени прошло с момента последнего детекта знака
            age = (now - self.last_sign_time).nanoseconds / 1e9

            # Знак считается активным если он был обнаружен недавно или команда от знака ненулевая
            sign_active = age < self.sign_timeout and (
                abs(self.last_sign_cmd.angular.z) > 1e-3 or abs(self.last_sign_cmd.linear.x) > 1e-3
            )
        # Если знак активен — он перехватывает управление
        if sign_active:
            # здесь берем угол который постит сам знак
